...
finder <- which(AF<0.05 | AF>0.95);
finderSNPs <- colnames(SNP.nobdata)[finder]
write.table(finderSNPs,file=bad_SNPs_low, quote=FALSE, row.names = FALSE, col.names = FALSE);

#remove bad stuff
SNPs.finder <- SNP.nobdata[,-finder];
ped.SNPs.finder <- matrix(NA, nrow=SNPpaitents,ncol=(2*ncol(SNPs.finder)))
for (i in 1:ncol(SNPs.finder)){
  NAind <- which(is.na(SNPs.finder[,i]));
  hom1ind <- which(SNPs.finder[,i]==0);
  hetind <- which(SNPs.finder[,i]==1);
  hom2ind <- which(SNPs.finder[,i]==2);
  
  newpedind1 <- i*2-1;
  newpedind2 <- i*2;
  
  ped.SNPs.finder[NAind,newpedind1] <- 0;
  ped.SNPs.finder[NAind, newpedind2] <- 0;
  ped.SNPs.finder[hom1ind, newpedind1] <- 1;
  ped.SNPs.finder[hom1ind, newpedind2] <- 1;
  ped.SNPs.finder[hetind, newpedind1] <- 1;
  ped.SNPs.finder[hetind, newpedind2] <- 2;
  ped.SNPs.finder[hom2ind, newpedind1] <- 2;
  ped.SNPs.finder[hom2ind, newpedind2] <- 2;
}

ped.filter <- cbind(SNPdata[1:6], ped.SNPs.finder);

file.create(file = 'h4pdf.pdf');
pdf('h4pdf');
for( i in 100){
  temp <- data.frame(phenotype=phenotyperead, ped.SNPs.finder[,i]);
  colnames(temp)[2] <- "SNP"
  plotting <- ggplot(data=temp) + geom_point(mapping=aes(x=SNP,y=phenotype,group=SNP))+labs(title=colnames(SNPs.finder)[i]);
  print(plotting)
}
dev.off();
